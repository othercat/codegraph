...
- On Windows, `codegraph init` and `codegraph index` now work reliably with Node.js 22.x. The built-in `node:sqlite` module on Windows was missing FTS5 support, which caused initialization to fail with "no such module: fts5"; CodeGraph now automatically falls back to `better-sqlite3` when FTS5 is unavailable, so Windows users no longer need WSL2 or a different Node version to get started.
